...
public:
	...
	/*Mat DetectEdges(Mat &src, Rect &roi,const int filter_size, const int low_threshold,
		const int ratio, const int kernel_size)
	{
		...
		blur(src_gray_roi, gray_blurred, filter_size, Point(-1, -1),BORDER_DEFAULT);
		Canny(gray_blurred, detected_edges)
		return src_copy;
	}*/
