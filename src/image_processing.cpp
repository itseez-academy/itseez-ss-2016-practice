...
cv::Mat ImageProcessorImpl::Pixelize(const cv::Mat &src,
    ...
    for(int i = 0; i < src_copy_roi.cols; i += x_block_size )
        for(int j = 0; j < src_copy_roi.rows; j += y_block_size) {
            cv::Mat src_roi_block = src_copy_roi(cv::Rect(i, j, x_block_size, y_block_size));
